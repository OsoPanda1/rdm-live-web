...
export function observeHistogram(
  ...
  const existing = getExistingHistogram(name, labels);
  const metric =
    existing ?? safeRegisterHistogram(name, DEFAULT_BUCKETS, labels);
...
export function recordError(
  ...
  incCounter("errors_by_code_total", 1, { service, code: errorCode, ...labels });
}
...
export function recordRequest(
  ...
  if (!success) {
    recordError("http", "request_failed", { endpoint: normalized, ...extraLabels });
  }
}
...
export function exportPrometheus(): string {
  ...
  // Histograms (clásicos Prometheus) [web:456][web:458][web:457][web:453]
  for (const metric of histograms.values()) {
    if (!typeEmitted.has(metric.name)) {
