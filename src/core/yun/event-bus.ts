import type { YunEventEnvelope, YunEventType, YunDomain, YunFederation, DataClassification } from './types';

type EventHandler<T = unknown> = (event: YunEventEnvelope<T>) => void | Promise<void>;

interface Subscription { id: string; eventType: string; handler: EventHandler; once: boolean; }
...
export function createEvent<T>(
  type: YunEventType, source: string, data: T,
  ...
  return {
    ...
    type, source, timestamp: new Date().toISOString(), data,
    metadata: { version: '1.0.0', correlationId: options.correlationId,
      causationId: options.causationId, federation: options.federation,
      domain: options.domain, classification: options.classification ?? 'internal' },
  };
}
...
export async function publish<T>(event: YunEventEnvelope<T>): Promise<void> {
  ...
  await Promise.allSettled(handlers.map(async (sub) => {
    try { await sub.handler(event); } catch (error) { console.error(`[YUN EventBus] Error:`, error); }
  }));
}
...
export function subscribe(eventType: string, handler: EventHandler, once = false): () => void {
  ...
  return () => {
    ...
    if (subs) { const idx = subs.findIndex((s) => s.id === id); if (idx >= 0) subs.splice(idx, 1); }
  };
}
...
export function getDeadLetterQueue(): YunEventEnvelope[] { return [...deadLetterQueue]; }

export function getSubscriptionCount(eventType?: string): number {
  if (eventType) return subscriptions.get(eventType)?.length ?? 0;
  let total = 0;
  for (const subs of subscriptions.values()) total += subs.length;
  return total;
}

export function clearDeadLetterQueue(): void { deadLetterQueue.length = 0; }
...
function isCriticalEvent(type: YunEventType): boolean {
  return type.includes('system.overload') || type.includes('system.mode-changed') ||
    type.includes('federation.degraded') || type.includes('federation.recovered');
}

export const YunEvents = {
  created<T>(domain: YunDomain, entity: string, data: T, source: string) {
    return publish(createEvent(`yun.${domain}.${entity}.created` as YunEventType, source, data, { domain }));
  },
  updated<T>(domain: YunDomain, entity: string, data: T, source: string) {
    return publish(createEvent(`yun.${domain}.${entity}.updated` as YunEventType, source, data, { domain }));
  },
  deleted<T>(domain: YunDomain, entity: string, data: T, source: string) {
    return publish(createEvent(`yun.${domain}.${entity}.deleted` as YunEventType, source, data, { domain }));
  },
